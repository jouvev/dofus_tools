...
class GuildInAllianceInformations(GuildInformations):
   ...
   def _nbMembersFunc(self,input) :
      ...
      if(self.nbMembers < 1 or self.nbMembers > 240) :
         raise RuntimeError("Forbidden value (" + self.nbMembers + ") on element of GuildInAllianceInformations.nbMembers.")
   
   def _joinDateFunc(self,input) :
      self.joinDate = input.readInt()
      if(self.joinDate < 0) :
         raise RuntimeError("Forbidden value (" + self.joinDate + ") on element of GuildInAllianceInformations.joinDate.")