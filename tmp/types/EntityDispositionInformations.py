class EntityDispositionInformations:
   ...
   def _cellIdFunc(self,input) :
      ...
      if(self.cellId < -1 or self.cellId > 559) :
         raise RuntimeError("Forbidden value (" + self.cellId + ") on element of EntityDispositionInformations.cellId.")
   
   def _directionFunc(self,input) :
      self.direction = input.readByte()
      if(self.direction < 0) :
         raise RuntimeError("Forbidden value (" + self.direction + ") on element of EntityDispositionInformations.direction.")