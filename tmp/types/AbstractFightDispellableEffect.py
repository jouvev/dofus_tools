class AbstractFightDispellableEffect:
   ...
   def _uidFunc(self,input) :
      ...
      if(self.uid < 0) :
         raise RuntimeError("Forbidden value (" + self.uid + ") on element of AbstractFightDispellableEffect.uid.")
   ...
   def _targetIdFunc(self,input) :
      ...
      if(self.targetId < -9007199254740992 or self.targetId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.targetId + ") on element of AbstractFightDispellableEffect.targetId.")
   ...
   def _dispelableFunc(self,input) :
      ...
      if(self.dispelable < 0) :
         raise RuntimeError("Forbidden value (" + self.dispelable + ") on element of AbstractFightDispellableEffect.dispelable.")
   
   def _spellIdFunc(self,input) :
      self.spellId = input.readVarUhShort()
      if(self.spellId < 0) :
         raise RuntimeError("Forbidden value (" + self.spellId + ") on element of AbstractFightDispellableEffect.spellId.")
   
   def _effectIdFunc(self,input) :
      self.effectId = input.readVarUhInt()
      if(self.effectId < 0) :
         raise RuntimeError("Forbidden value (" + self.effectId + ") on element of AbstractFightDispellableEffect.effectId.")
   
   def _parentBoostUidFunc(self,input) :
      self.parentBoostUid = input.readVarUhInt()
      if(self.parentBoostUid < 0) :
         raise RuntimeError("Forbidden value (" + self.parentBoostUid + ") on element of AbstractFightDispellableEffect.parentBoostUid.")