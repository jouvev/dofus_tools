...
class GameFightTaxCollectorInformations(GameFightAIInformations):
   ...
   def _firstNameIdFunc(self,input) :
      ...
      if(self.firstNameId < 0) :
         raise RuntimeError("Forbidden value (" + self.firstNameId + ") on element of GameFightTaxCollectorInformations.firstNameId.")
   
   def _lastNameIdFunc(self,input) :
      self.lastNameId = input.readVarUhShort()
      if(self.lastNameId < 0) :
         raise RuntimeError("Forbidden value (" + self.lastNameId + ") on element of GameFightTaxCollectorInformations.lastNameId.")
   
   def _levelFunc(self,input) :
      self.level = input.readUnsignedByte()
      if(self.level < 0 or self.level > 255) :
         raise RuntimeError("Forbidden value (" + self.level + ") on element of GameFightTaxCollectorInformations.level.")