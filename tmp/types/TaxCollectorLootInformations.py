...
class TaxCollectorLootInformations(TaxCollectorComplementaryInformations):
   ...
   def _kamasFunc(self,input) :
      ...
      if(self.kamas < 0 or self.kamas > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.kamas + ") on element of TaxCollectorLootInformations.kamas.")
   ...
   def _experienceFunc(self,input) :
      ...
      if(self.experience < 0 or self.experience > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.experience + ") on element of TaxCollectorLootInformations.experience.")
   
   def _podsFunc(self,input) :
      self.pods = input.readVarUhInt()
      if(self.pods < 0) :
         raise RuntimeError("Forbidden value (" + self.pods + ") on element of TaxCollectorLootInformations.pods.")
   ...
   def _itemsValueFunc(self,input) :
      ...
      if(self.itemsValue < 0 or self.itemsValue > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.itemsValue + ") on element of TaxCollectorLootInformations.itemsValue.")