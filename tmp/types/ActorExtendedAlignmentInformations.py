...
class ActorExtendedAlignmentInformations(ActorAlignmentInformations):
   ...
   def _honorFunc(self,input) :
      ...
      if(self.honor < 0 or self.honor > 20000) :
         raise RuntimeError("Forbidden value (" + self.honor + ") on element of ActorExtendedAlignmentInformations.honor.")
   ...
   def _honorGradeFloorFunc(self,input) :
      ...
      if(self.honorGradeFloor < 0 or self.honorGradeFloor > 20000) :
         raise RuntimeError("Forbidden value (" + self.honorGradeFloor + ") on element of ActorExtendedAlignmentInformations.honorGradeFloor.")
   ...
   def _honorNextGradeFloorFunc(self,input) :
      ...
      if(self.honorNextGradeFloor < 0 or self.honorNextGradeFloor > 20000) :
         raise RuntimeError("Forbidden value (" + self.honorNextGradeFloor + ") on element of ActorExtendedAlignmentInformations.honorNextGradeFloor.")
   
   def _aggressableFunc(self,input) :
      self.aggressable = input.readByte()
      if(self.aggressable < 0) :
         raise RuntimeError("Forbidden value (" + self.aggressable + ") on element of ActorExtendedAlignmentInformations.aggressable.")