...
class FightTeamMemberTaxCollectorInformations(FightTeamMemberInformations):
   ...
   def _firstNameIdFunc(self,input) :
      ...
      if(self.firstNameId < 0) :
         raise RuntimeError("Forbidden value (" + self.firstNameId + ") on element of FightTeamMemberTaxCollectorInformations.firstNameId.")
   
   def _lastNameIdFunc(self,input) :
      self.lastNameId = input.readVarUhShort()
      if(self.lastNameId < 0) :
         raise RuntimeError("Forbidden value (" + self.lastNameId + ") on element of FightTeamMemberTaxCollectorInformations.lastNameId.")
   
   def _levelFunc(self,input) :
      self.level = input.readUnsignedByte()
      if(self.level < 1 or self.level > 200) :
         raise RuntimeError("Forbidden value (" + self.level + ") on element of FightTeamMemberTaxCollectorInformations.level.")
   
   def _guildIdFunc(self,input) :
      self.guildId = input.readVarUhInt()
      if(self.guildId < 0) :
         raise RuntimeError("Forbidden value (" + self.guildId + ") on element of FightTeamMemberTaxCollectorInformations.guildId.")
   
   def _uidFunc(self,input) :
      self.uid = input.readDouble()
      if(self.uid < 0 or self.uid > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.uid + ") on element of FightTeamMemberTaxCollectorInformations.uid.")