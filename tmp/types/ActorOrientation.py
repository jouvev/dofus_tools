class ActorOrientation:
   ...
   def _idFunc(self,input) :
      ...
      if(self.id < -9007199254740992 or self.id > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.id + ") on element of ActorOrientation.id.")
   
   def _directionFunc(self,input) :
      self.direction = input.readByte()
      if(self.direction < 0) :
         raise RuntimeError("Forbidden value (" + self.direction + ") on element of ActorOrientation.direction.")