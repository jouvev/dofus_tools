class Idol:
   ...
   def _idFunc(self,input) :
      ...
      if(self.id < 0) :
         raise RuntimeError("Forbidden value (" + self.id + ") on element of Idol.id.")
   
   def _xpBonusPercentFunc(self,input) :
      self.xpBonusPercent = input.readVarUhShort()
      if(self.xpBonusPercent < 0) :
         raise RuntimeError("Forbidden value (" + self.xpBonusPercent + ") on element of Idol.xpBonusPercent.")
   
   def _dropBonusPercentFunc(self,input) :
      self.dropBonusPercent = input.readVarUhShort()
      if(self.dropBonusPercent < 0) :
         raise RuntimeError("Forbidden value (" + self.dropBonusPercent + ") on element of Idol.dropBonusPercent.")