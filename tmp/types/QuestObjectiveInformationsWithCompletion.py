...
class QuestObjectiveInformationsWithCompletion(QuestObjectiveInformations):
   ...
   def _curCompletionFunc(self,input) :
      ...
      if(self.curCompletion < 0) :
         raise RuntimeError("Forbidden value (" + self.curCompletion + ") on element of QuestObjectiveInformationsWithCompletion.curCompletion.")
   
   def _maxCompletionFunc(self,input) :
      self.maxCompletion = input.readVarUhShort()
      if(self.maxCompletion < 0) :
         raise RuntimeError("Forbidden value (" + self.maxCompletion + ") on element of QuestObjectiveInformationsWithCompletion.maxCompletion.")