class ArenaRanking:
   ...
   def _rankFunc(self,input) :
      ...
      if(self.rank < 0 or self.rank > 20000) :
         raise RuntimeError("Forbidden value (" + str(self.rank) + ") on element of ArenaRanking.rank.")
   
   def _bestRankFunc(self,input) :
      self.bestRank = input.readVarUhShort()
      if(self.bestRank < 0 or self.bestRank > 20000) :
         raise RuntimeError("Forbidden value (" + str(self.bestRank) + ") on element of ArenaRanking.bestRank.")

   def resume(self):
      print("rank :",self.rank)
      print("bestRank :",self.bestRank)
