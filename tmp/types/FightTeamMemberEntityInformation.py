...
class FightTeamMemberEntityInformation(FightTeamMemberInformations):
   ...
   def _entityModelIdFunc(self,input) :
      ...
      if(self.entityModelId < 0) :
         raise RuntimeError("Forbidden value (" + self.entityModelId + ") on element of FightTeamMemberEntityInformation.entityModelId.")
   
   def _levelFunc(self,input) :
      self.level = input.readVarUhShort()
      if(self.level < 1 or self.level > 200) :
         raise RuntimeError("Forbidden value (" + self.level + ") on element of FightTeamMemberEntityInformation.level.")
   ...
   def _masterIdFunc(self,input) :
      ...
      if(self.masterId < -9007199254740992 or self.masterId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.masterId + ") on element of FightTeamMemberEntityInformation.masterId.")