...
class HumanOptionOrnament(HumanOption):
   ...
   def _ornamentIdFunc(self,input) :
      ...
      if(self.ornamentId < 0) :
         raise RuntimeError("Forbidden value (" + self.ornamentId + ") on element of HumanOptionOrnament.ornamentId.")
   
   def _levelFunc(self,input) :
      self.level = input.readVarUhShort()
      if(self.level < 0) :
         raise RuntimeError("Forbidden value (" + self.level + ") on element of HumanOptionOrnament.level.")
