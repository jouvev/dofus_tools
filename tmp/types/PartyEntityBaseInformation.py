...
class PartyEntityBaseInformation:
   ...
   def _indexIdFunc(self,input) :
      ...
      if(self.indexId < 0) :
         raise RuntimeError("Forbidden value (" + self.indexId + ") on element of PartyEntityBaseInformation.indexId.")
   
   def _entityModelIdFunc(self,input) :
      self.entityModelId = input.readByte()
      if(self.entityModelId < 0) :
         raise RuntimeError("Forbidden value (" + self.entityModelId + ") on element of PartyEntityBaseInformation.entityModelId.")