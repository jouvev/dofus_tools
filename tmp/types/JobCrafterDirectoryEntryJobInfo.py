class JobCrafterDirectoryEntryJobInfo:
   ...
   def _jobIdFunc(self,input) :
      ...
      if(self.jobId < 0) :
         raise RuntimeError("Forbidden value (" + self.jobId + ") on element of JobCrafterDirectoryEntryJobInfo.jobId.")
   
   def _jobLevelFunc(self,input) :
      self.jobLevel = input.readUnsignedByte()
      if(self.jobLevel < 1 or self.jobLevel > 200) :
         raise RuntimeError("Forbidden value (" + self.jobLevel + ") on element of JobCrafterDirectoryEntryJobInfo.jobLevel.")
   ...
   def _minLevelFunc(self,input) :
      ...
      if(self.minLevel < 0 or self.minLevel > 255) :
         raise RuntimeError("Forbidden value (" + self.minLevel + ") on element of JobCrafterDirectoryEntryJobInfo.minLevel.")