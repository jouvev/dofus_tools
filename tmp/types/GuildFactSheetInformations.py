...
class GuildFactSheetInformations(GuildInformations):
   ...
   def _leaderIdFunc(self,input) :
      ...
      if(self.leaderId < 0 or self.leaderId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.leaderId + ") on element of GuildFactSheetInformations.leaderId.")
   
   def _nbMembersFunc(self,input) :
      self.nbMembers = input.readVarUhShort()
      if(self.nbMembers < 0) :
         raise RuntimeError("Forbidden value (" + self.nbMembers + ") on element of GuildFactSheetInformations.nbMembers.")
   
   def _lastActivityDayFunc(self,input) :
      self.lastActivityDay = input.readShort()
      if(self.lastActivityDay < 0) :
         raise RuntimeError("Forbidden value (" + self.lastActivityDay + ") on element of GuildFactSheetInformations.lastActivityDay.")
   
   def _nbPendingApplyFunc(self,input) :
      self.nbPendingApply = input.readInt()
      if(self.nbPendingApply < 0) :
         raise RuntimeError("Forbidden value (" + self.nbPendingApply + ") on element of GuildFactSheetInformations.nbPendingApply.")