...
class SpawnCompanionInformation(SpawnInformation):
   ...
   def _modelIdFunc(self,input) :
      ...
      if(self.modelId < 0) :
         raise RuntimeError("Forbidden value (" + self.modelId + ") on element of SpawnCompanionInformation.modelId.")
   
   def _levelFunc(self,input) :
      self.level = input.readVarUhShort()
      if(self.level < 1 or self.level > 200) :
         raise RuntimeError("Forbidden value (" + self.level + ") on element of SpawnCompanionInformation.level.")
   ...
   def _summonerIdFunc(self,input) :
      ...
      if(self.summonerId < -9007199254740992 or self.summonerId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.summonerId + ") on element of SpawnCompanionInformation.summonerId.")
   ...
   def _ownerIdFunc(self,input) :
      ...
      if(self.ownerId < -9007199254740992 or self.ownerId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.ownerId + ") on element of SpawnCompanionInformation.ownerId.")