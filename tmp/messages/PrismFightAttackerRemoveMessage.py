class PrismFightAttackerRemoveMessage:
   ...
   def _subAreaIdFunc(self,input) :
      ...
      if(self.subAreaId < 0) :
         raise RuntimeError("Forbidden value (" + self.subAreaId + ") on element of PrismFightAttackerRemoveMessage.subAreaId.")
   
   def _fightIdFunc(self,input) :
      self.fightId = input.readVarUhShort()
      if(self.fightId < 0) :
         raise RuntimeError("Forbidden value (" + self.fightId + ") on element of PrismFightAttackerRemoveMessage.fightId.")
   ...
   def _fighterToRemoveIdFunc(self,input) :
      ...
      if(self.fighterToRemoveId < 0 or self.fighterToRemoveId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.fighterToRemoveId + ") on element of PrismFightAttackerRemoveMessage.fighterToRemoveId.")