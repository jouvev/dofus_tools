...
class GameActionFightCarryCharacterMessage(AbstractGameActionMessage):
   ...
   def _targetIdFunc(self,input) :
      ...
      if(self.targetId < -9007199254740992 or self.targetId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.targetId + ") on element of GameActionFightCarryCharacterMessage.targetId.")
   
   def _cellIdFunc(self,input) :
      self.cellId = input.readShort()
      if(self.cellId < -1 or self.cellId > 559) :
         raise RuntimeError("Forbidden value (" + self.cellId + ") on element of GameActionFightCarryCharacterMessage.cellId.")