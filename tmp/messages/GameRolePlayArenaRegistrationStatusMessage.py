class GameRolePlayArenaRegistrationStatusMessage:
   ...
   def _stepFunc(self,input) :
      ...
      if(self.step < 0) :
         raise RuntimeError("Forbidden value (" + self.step + ") on element of GameRolePlayArenaRegistrationStatusMessage.step.")
   
   def _battleModeFunc(self,input) :
      self.battleMode = input.readInt()
      if(self.battleMode < 0) :
         raise RuntimeError("Forbidden value (" + self.battleMode + ") on element of GameRolePlayArenaRegistrationStatusMessage.battleMode.")