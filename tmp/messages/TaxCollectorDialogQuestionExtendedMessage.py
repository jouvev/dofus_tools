...
class TaxCollectorDialogQuestionExtendedMessage(TaxCollectorDialogQuestionBasicMessage):
   ...
   def _maxPodsFunc(self,input) :
      ...
      if(self.maxPods < 0) :
         raise RuntimeError("Forbidden value (" + self.maxPods + ") on element of TaxCollectorDialogQuestionExtendedMessage.maxPods.")
   
   def _prospectingFunc(self,input) :
      self.prospecting = input.readVarUhShort()
      if(self.prospecting < 0) :
         raise RuntimeError("Forbidden value (" + self.prospecting + ") on element of TaxCollectorDialogQuestionExtendedMessage.prospecting.")
   
   def _wisdomFunc(self,input) :
      self.wisdom = input.readVarUhShort()
      if(self.wisdom < 0) :
         raise RuntimeError("Forbidden value (" + self.wisdom + ") on element of TaxCollectorDialogQuestionExtendedMessage.wisdom.")
   
   def _taxCollectorsCountFunc(self,input) :
      self.taxCollectorsCount = input.readByte()
      if(self.taxCollectorsCount < 0) :
         raise RuntimeError("Forbidden value (" + self.taxCollectorsCount + ") on element of TaxCollectorDialogQuestionExtendedMessage.taxCollectorsCount.")
   ...
   def _kamasFunc(self,input) :
      ...
      if(self.kamas < 0 or self.kamas > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.kamas + ") on element of TaxCollectorDialogQuestionExtendedMessage.kamas.")
   ...
   def _experienceFunc(self,input) :
      ...
      if(self.experience < 0 or self.experience > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.experience + ") on element of TaxCollectorDialogQuestionExtendedMessage.experience.")
   
   def _podsFunc(self,input) :
      self.pods = input.readVarUhInt()
      if(self.pods < 0) :
         raise RuntimeError("Forbidden value (" + self.pods + ") on element of TaxCollectorDialogQuestionExtendedMessage.pods.")
   ...
   def _itemsValueFunc(self,input) :
      ...
      if(self.itemsValue < 0 or self.itemsValue > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.itemsValue + ") on element of TaxCollectorDialogQuestionExtendedMessage.itemsValue.")