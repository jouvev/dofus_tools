...
class ExchangePodsModifiedMessage(ExchangeObjectMessage):
   ...
   def _currentWeightFunc(self,input) :
      ...
      if(self.currentWeight < 0) :
         raise RuntimeError("Forbidden value (" + self.currentWeight + ") on element of ExchangePodsModifiedMessage.currentWeight.")
   
   def _maxWeightFunc(self,input) :
      self.maxWeight = input.readVarUhInt()
      if(self.maxWeight < 0) :
         raise RuntimeError("Forbidden value (" + self.maxWeight + ") on element of ExchangePodsModifiedMessage.maxWeight.")