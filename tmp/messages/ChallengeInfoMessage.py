class ChallengeInfoMessage:
   ...
   def _challengeIdFunc(self,input) :
      ...
      if(self.challengeId < 0) :
         raise RuntimeError("Forbidden value (" + self.challengeId + ") on element of ChallengeInfoMessage.challengeId.")
   ...
   def _targetIdFunc(self,input) :
      ...
      if(self.targetId < -9007199254740992 or self.targetId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.targetId + ") on element of ChallengeInfoMessage.targetId.")
   
   def _xpBonusFunc(self,input) :
      self.xpBonus = input.readVarUhInt()
      if(self.xpBonus < 0) :
         raise RuntimeError("Forbidden value (" + self.xpBonus + ") on element of ChallengeInfoMessage.xpBonus.")
   
   def _dropBonusFunc(self,input) :
      self.dropBonus = input.readVarUhInt()
      if(self.dropBonus < 0) :
         raise RuntimeError("Forbidden value (" + self.dropBonus + ") on element of ChallengeInfoMessage.dropBonus.")