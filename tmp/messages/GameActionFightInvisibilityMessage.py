...
class GameActionFightInvisibilityMessage(AbstractGameActionMessage):
   ...
   def _targetIdFunc(self,input) :
      ...
      if(self.targetId < -9007199254740992 or self.targetId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.targetId + ") on element of GameActionFightInvisibilityMessage.targetId.")
   
   def _stateFunc(self,input) :
      self.state = input.readByte()
      if(self.state < 0) :
         raise RuntimeError("Forbidden value (" + self.state + ") on element of GameActionFightInvisibilityMessage.state.")