...
class PaddockToSellListMessage:
   ...
   def _pageIndexFunc(self,input) :
      ...
      if(self.pageIndex < 0) :
         raise RuntimeError("Forbidden value (" + self.pageIndex + ") on element of PaddockToSellListMessage.pageIndex.")
   
   def _totalPageFunc(self,input) :
      self.totalPage = input.readVarUhShort()
      if(self.totalPage < 0) :
         raise RuntimeError("Forbidden value (" + self.totalPage + ") on element of PaddockToSellListMessage.totalPage.")