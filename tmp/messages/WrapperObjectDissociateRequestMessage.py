class WrapperObjectDissociateRequestMessage:
   ...
   def _hostUIDFunc(self,input) :
      ...
      if(self.hostUID < 0) :
         raise RuntimeError("Forbidden value (" + self.hostUID + ") on element of WrapperObjectDissociateRequestMessage.hostUID.")
   
   def _hostPosFunc(self,input) :
      self.hostPos = input.readUnsignedByte()
      if(self.hostPos < 0 or self.hostPos > 255) :
         raise RuntimeError("Forbidden value (" + self.hostPos + ") on element of WrapperObjectDissociateRequestMessage.hostPos.")