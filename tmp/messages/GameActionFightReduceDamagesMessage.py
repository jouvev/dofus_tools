...
class GameActionFightReduceDamagesMessage(AbstractGameActionMessage):
   ...
   def _targetIdFunc(self,input) :
      ...
      if(self.targetId < -9007199254740992 or self.targetId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.targetId + ") on element of GameActionFightReduceDamagesMessage.targetId.")
   
   def _amountFunc(self,input) :
      self.amount = input.readVarUhInt()
      if(self.amount < 0) :
         raise RuntimeError("Forbidden value (" + self.amount + ") on element of GameActionFightReduceDamagesMessage.amount.")