...
class PartyUpdateLightMessage(AbstractPartyEventMessage):
   ...
   def _idFunc(self,input) :
      ...
      if(self.id < 0 or self.id > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.id + ") on element of PartyUpdateLightMessage.id.")
   
   def _lifePointsFunc(self,input) :
      self.lifePoints = input.readVarUhInt()
      if(self.lifePoints < 0) :
         raise RuntimeError("Forbidden value (" + self.lifePoints + ") on element of PartyUpdateLightMessage.lifePoints.")
   
   def _maxLifePointsFunc(self,input) :
      self.maxLifePoints = input.readVarUhInt()
      if(self.maxLifePoints < 0) :
         raise RuntimeError("Forbidden value (" + self.maxLifePoints + ") on element of PartyUpdateLightMessage.maxLifePoints.")
   
   def _prospectingFunc(self,input) :
      self.prospecting = input.readVarUhInt()
      if(self.prospecting < 0) :
         raise RuntimeError("Forbidden value (" + self.prospecting + ") on element of PartyUpdateLightMessage.prospecting.")
   
   def _regenRateFunc(self,input) :
      self.regenRate = input.readUnsignedByte()
      if(self.regenRate < 0 or self.regenRate > 255) :
         raise RuntimeError("Forbidden value (" + self.regenRate + ") on element of PartyUpdateLightMessage.regenRate.")