class GameFightTurnStartMessage:
   ...
   def _idFunc(self,input) :
      ...
      if(self.id < -9007199254740992 or self.id > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.id + ") on element of GameFightTurnStartMessage.id.")
   
   def _waitTimeFunc(self,input) :
      self.waitTime = input.readVarUhInt()
      if(self.waitTime < 0) :
         raise RuntimeError("Forbidden value (" + self.waitTime + ") on element of GameFightTurnStartMessage.waitTime.")