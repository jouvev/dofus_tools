class ActivitySuggestionsRequestMessage:
   ...
   def _minLevelFunc(self,input) :
      ...
      if(self.minLevel < 0) :
         raise RuntimeError("Forbidden value (" + self.minLevel + ") on element of ActivitySuggestionsRequestMessage.minLevel.")
   
   def _maxLevelFunc(self,input) :
      self.maxLevel = input.readVarUhShort()
      if(self.maxLevel < 0) :
         raise RuntimeError("Forbidden value (" + self.maxLevel + ") on element of ActivitySuggestionsRequestMessage.maxLevel.")
   
   def _areaIdFunc(self,input) :
      self.areaId = input.readVarUhShort()
      if(self.areaId < 0) :
         raise RuntimeError("Forbidden value (" + self.areaId + ") on element of ActivitySuggestionsRequestMessage.areaId.")
   
   def _activityCategoryIdFunc(self,input) :
      self.activityCategoryId = input.readVarUhShort()
      if(self.activityCategoryId < 0) :
         raise RuntimeError("Forbidden value (" + self.activityCategoryId + ") on element of ActivitySuggestionsRequestMessage.activityCategoryId.")
   
   def _nbCardsFunc(self,input) :
      self.nbCards = input.readUnsignedShort()
      if(self.nbCards < 0 or self.nbCards > 65535) :
         raise RuntimeError("Forbidden value (" + self.nbCards + ") on element of ActivitySuggestionsRequestMessage.nbCards.")