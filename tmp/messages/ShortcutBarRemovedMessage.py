class ShortcutBarRemovedMessage:
   ...
   def _barTypeFunc(self,input) :
      ...
      if(self.barType < 0) :
         raise RuntimeError("Forbidden value (" + self.barType + ") on element of ShortcutBarRemovedMessage.barType.")
   
   def _slotFunc(self,input) :
      self.slot = input.readByte()
      if(self.slot < 0 or self.slot > 99) :
         raise RuntimeError("Forbidden value (" + self.slot + ") on element of ShortcutBarRemovedMessage.slot.")