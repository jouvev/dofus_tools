...
class TaxCollectorListMessage(AbstractTaxCollectorListMessage):
   ...
   def _nbcollectorMaxFunc(self,input) :
      ...
      if(self.nbcollectorMax < 0) :
         raise RuntimeError("Forbidden value (" + self.nbcollectorMax + ") on element of TaxCollectorListMessage.nbcollectorMax.")
   
   def _infoTypeFunc(self,input) :
      self.infoType = input.readByte()
      if(self.infoType < 0) :
         raise RuntimeError("Forbidden value (" + self.infoType + ") on element of TaxCollectorListMessage.infoType.")