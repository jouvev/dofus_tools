class GameRolePlayPlayerFightFriendlyAnsweredMessage:
   ...
   def _fightIdFunc(self,input) :
      ...
      if(self.fightId < 0) :
         raise RuntimeError("Forbidden value (" + self.fightId + ") on element of GameRolePlayPlayerFightFriendlyAnsweredMessage.fightId.")
   
   def _sourceIdFunc(self,input) :
      self.sourceId = input.readVarUhLong()
      if(self.sourceId < 0 or self.sourceId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.sourceId + ") on element of GameRolePlayPlayerFightFriendlyAnsweredMessage.sourceId.")
   
   def _targetIdFunc(self,input) :
      self.targetId = input.readVarUhLong()
      if(self.targetId < 0 or self.targetId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.targetId + ") on element of GameRolePlayPlayerFightFriendlyAnsweredMessage.targetId.")
