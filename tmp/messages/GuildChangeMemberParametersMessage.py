class GuildChangeMemberParametersMessage:
   ...
   def _memberIdFunc(self,input) :
      ...
      if(self.memberId < 0 or self.memberId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.memberId + ") on element of GuildChangeMemberParametersMessage.memberId.")
   
   def _rankIdFunc(self,input) :
      self.rankId = input.readVarUhInt()
      if(self.rankId < 0) :
         raise RuntimeError("Forbidden value (" + self.rankId + ") on element of GuildChangeMemberParametersMessage.rankId.")
   ...
   def _experienceGivenPercentFunc(self,input) :
      ...
      if(self.experienceGivenPercent < 0 or self.experienceGivenPercent > 100) :
         raise RuntimeError("Forbidden value (" + self.experienceGivenPercent + ") on element of GuildChangeMemberParametersMessage.experienceGivenPercent.")