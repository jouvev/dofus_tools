class HaapiConfirmationMessage:
   ...
   def _kamasFunc(self,input) :
      ...
      if(self.kamas < 0 or self.kamas > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.kamas + ") on element of HaapiConfirmationMessage.kamas.")
   
   def _amountFunc(self,input) :
      self.amount = input.readVarUhLong()
      if(self.amount < 0 or self.amount > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.amount + ") on element of HaapiConfirmationMessage.amount.")
   
   def _rateFunc(self,input) :
      self.rate = input.readVarUhShort()
      if(self.rate < 0) :
         raise RuntimeError("Forbidden value (" + self.rate + ") on element of HaapiConfirmationMessage.rate.")
   
   def _actionFunc(self,input) :
      self.action = input.readByte()
      if(self.action < 0) :
         raise RuntimeError("Forbidden value (" + self.action + ") on element of HaapiConfirmationMessage.action.")
