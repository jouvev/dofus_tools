class GameRolePlaySpellAnimMessage:
   ...
   def _casterIdFunc(self,input) :
      ...
      if(self.casterId < 0 or self.casterId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.casterId + ") on element of GameRolePlaySpellAnimMessage.casterId.")
   
   def _targetCellIdFunc(self,input) :
      self.targetCellId = input.readVarUhShort()
      if(self.targetCellId < 0 or self.targetCellId > 559) :
         raise RuntimeError("Forbidden value (" + self.targetCellId + ") on element of GameRolePlaySpellAnimMessage.targetCellId.")
   
   def _spellIdFunc(self,input) :
      self.spellId = input.readVarUhShort()
      if(self.spellId < 0) :
         raise RuntimeError("Forbidden value (" + self.spellId + ") on element of GameRolePlaySpellAnimMessage.spellId.")
   
   def _spellLevelFunc(self,input) :
      self.spellLevel = input.readShort()
      if(self.spellLevel < 1 or self.spellLevel > 32767) :
         raise RuntimeError("Forbidden value (" + self.spellLevel + ") on element of GameRolePlaySpellAnimMessage.spellLevel.")
   
   def _directionFunc(self,input) :
      self.direction = input.readShort()
      if(self.direction < -1 or self.direction > 8) :
         raise RuntimeError("Forbidden value (" + self.direction + ") on element of GameRolePlaySpellAnimMessage.direction.")