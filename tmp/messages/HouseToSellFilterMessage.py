class HouseToSellFilterMessage:
   ...
   def _atLeastNbRoomFunc(self,input) :
      ...
      if(self.atLeastNbRoom < 0) :
         raise RuntimeError("Forbidden value (" + self.atLeastNbRoom + ") on element of HouseToSellFilterMessage.atLeastNbRoom.")
   
   def _atLeastNbChestFunc(self,input) :
      self.atLeastNbChest = input.readByte()
      if(self.atLeastNbChest < 0) :
         raise RuntimeError("Forbidden value (" + self.atLeastNbChest + ") on element of HouseToSellFilterMessage.atLeastNbChest.")
   
   def _skillRequestedFunc(self,input) :
      self.skillRequested = input.readVarUhShort()
      if(self.skillRequested < 0) :
         raise RuntimeError("Forbidden value (" + self.skillRequested + ") on element of HouseToSellFilterMessage.skillRequested.")
   
   def _maxPriceFunc(self,input) :
      self.maxPrice = input.readVarUhLong()
      if(self.maxPrice < 0 or self.maxPrice > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.maxPrice + ") on element of HouseToSellFilterMessage.maxPrice.")
   
   def _orderByFunc(self,input) :
      self.orderBy = input.readByte()
      if(self.orderBy < 0) :
         raise RuntimeError("Forbidden value (" + self.orderBy + ") on element of HouseToSellFilterMessage.orderBy.")