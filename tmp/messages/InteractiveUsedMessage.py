class InteractiveUsedMessage:
   ...
   def _entityIdFunc(self,input) :
      ...
      if(self.entityId < 0 or self.entityId > 9007199254740992) :
         raise RuntimeError("Forbidden value (" + self.entityId + ") on element of InteractiveUsedMessage.entityId.")
   
   def _elemIdFunc(self,input) :
      self.elemId = input.readVarUhInt()
      if(self.elemId < 0) :
         raise RuntimeError("Forbidden value (" + self.elemId + ") on element of InteractiveUsedMessage.elemId.")
   
   def _skillIdFunc(self,input) :
      self.skillId = input.readVarUhShort()
      if(self.skillId < 0) :
         raise RuntimeError("Forbidden value (" + self.skillId + ") on element of InteractiveUsedMessage.skillId.")
   
   def _durationFunc(self,input) :
      self.duration = input.readVarUhShort()
      if(self.duration < 0) :
         raise RuntimeError("Forbidden value (" + self.duration + ") on element of InteractiveUsedMessage.duration.")
