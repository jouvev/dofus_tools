class ExchangeMountsTakenFromPaddockMessage:
   ...
   def _worldXFunc(self,input) :
      ...
      if(self.worldX < -255 or self.worldX > 255) :
         raise RuntimeError("Forbidden value (" + self.worldX + ") on element of ExchangeMountsTakenFromPaddockMessage.worldX.")
   
   def _worldYFunc(self,input) :
      self.worldY = input.readShort()
      if(self.worldY < -255 or self.worldY > 255) :
         raise RuntimeError("Forbidden value (" + self.worldY + ") on element of ExchangeMountsTakenFromPaddockMessage.worldY.")
